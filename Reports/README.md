# Reports

The Privacy Platform provides several system, tenant and application level reports to allow you to get the most use and information from your subject activities.

## System Reports

-   [Summary](System_Summary.md)
-   [Active Subjects](System_ActiveSubjects.md)

## Tenant Reports

-   [View Logs](Tenant_Logs.md)

## Application Reports

-   [View Users](Applications_Users.md)
-   [Consents](Applications_Consents.md)
-   [View Logs](Applications_Logs.md)
-   [View Subjects](Applications_Subjects.md)
-   [View Demographics](Applications_Demographics.md)
-   [View Application Message Costs](Applications_MessageCosts.md)
-   [View Subject Requests](Applications_SubjectRequests.md)
-   [View Pending Tasks](Applications_PendingTasks.md)
-   [View Application Requests](Applications_ApplicationRequests.md)